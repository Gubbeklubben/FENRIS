...
from fedbench.util.metrics import make_tabular_preprocessor, get_quasi_identifiers, get_schema_columns
...
class DirectOverlapDiagnosticEvaluator(Evaluator):
    def evaluate(self, ctx: EvalContext) -> dict[str, float]:
        ...
        H_train = set(_row_hash(train[common]))
        H_syn = _row_hash(syn[common])
        ...
        for k in (1, 2, 3):
            cols = ranked[:k]
            Ht = set(_row_hash(train[cols]))
            Hs = _row_hash(syn[cols])
    ...
    def evaluate(self, ctx: EvalContext) -> dict[str, float]:
        ...
        for sensitive_column in ctx.sensitive_columns or []:
            ...
            if ctx.schema.kind_of(sensitive_column) in ["binary", "categorical"]:
                ...
                model.fit(X_syn, y_syn)
                y_pred = model.predict(X_test)
                metrics[f"aia_rmse.{sensitive_column_normalized}"] = sqrt(mean_squared_error(y_test, y_pred))
