...
from fedtabdiff_modules import init_model, train_model, generate_samples, \
    decode_samples
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        log(INFO,
            f"[Client {self.cid}] fit, server round: {server_round}, n_samples: {n_samples}")
    ...
    def evaluate(self, parameters, config):
        ...
        if (server_round % self.exp_params['eval_rate_client'] == 0) and (
                server_round > 0):
            log(INFO,
                f"[Client {self.cid}] evaluate, server round: {server_round}, n_samples: {n_samples}")
            # initialize model
            set_parameters(self.synthesizer, parameters)
        ...
        log(INFO,
            f"[Client {self.cid}] fidelity {fidelity_score.get('fidelity')}")
