...
from fedbench.flwr.client_context import build_client_context
...
def recv_config(message: Message, flwr_context: Context) -> Message:
    cache = ClientCacheWrapper(flwr_context.state)
    cache.set_config(message.content.config_records["config"])
...
def recv_artifacts(message: Message, flwr_context: Context) -> Message:
    cache = ClientCacheWrapper(flwr_context.state)
    cache.set_artifacts(message.content)
    return Message(content=RecordDict(), reply_to=message)
...
def fed_init(message: Message, flwr_context: Context) -> Message:
    cache = ClientCacheWrapper(flwr_context.state)
    ctx = build_client_context(cache)
    partition_id = _get_partition_id(flwr_context)
    ...
    reply = synthesizer.fed_init(request, ctx.config.seed, ctx.dataset.schema, train_df)
