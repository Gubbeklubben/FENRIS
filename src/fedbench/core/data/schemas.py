...
FEDBENCH_SCHEMA_FORMAT_IDENTIFIER = "FedbenchSchema"
FEDBENCH_SCHEMA_FORMAT_VERSION = "1.0.0"
...
@dataclass(frozen=True)
class TableSchema:
    schema_format: str = FEDBENCH_SCHEMA_FORMAT_IDENTIFIER
    schema_format_version: str = FEDBENCH_SCHEMA_FORMAT_VERSION
...
def _load_schema(schema_path: Path) -> TableSchema:
    ...
    if root["schema_format"] != FEDBENCH_SCHEMA_FORMAT_IDENTIFIER:
        raise ValueError(f"Schema format {root['schema_format']} is not supported.")
    if root["schema_format_version"] != FEDBENCH_SCHEMA_FORMAT_VERSION:
        raise ValueError(
            f"Fedbench schema version {root['schema_format_version']} is not supported."
        )
