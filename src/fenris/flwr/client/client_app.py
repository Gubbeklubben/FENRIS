...
from fenris.core.algorithm import SampleContext, TrainContext
from fenris.core.encoder import FenrisEncoder
from fenris.core.eval import LocalEvalContext
from fenris.core.payload import Extras, Payload
from fenris.flwr.client.context import build_client_context
from fenris.flwr.namespace import Namespace
...
def evaluate(message: Message, flwr_context: Context) -> Message:
    ...
    for key, value in ctx.eval_suite.local_evaluate(eval_ctx).items():
        try:
            metrics[key] = json.dumps(value, cls=FenrisEncoder)
